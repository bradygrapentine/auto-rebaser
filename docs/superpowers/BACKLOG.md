...
_Last `/backlog-sync`: 2026-05-30 (OPS-2 **stage (b)** shipped #223 — vitest 2.1.9→3.2.4, vite held at 5. Earlier: stage (a) #221, SEC-5 #219, PERF-1 #216, SEC-7 #217, OPS-2 OSV time-box #215, OPS-1 #211, OPS-3 #213. OPS-2 now Ready at **stage (c)** [vite 5→6 + esbuild — the slice that clears the OSV allowlist]; Low/de-urgentized. New candidate: COVERAGE-1 in §5.)_
...
**Status:** 🟢 Ready at **stage (c)** (DE-URGENTIZED 2026-05-29 — the blocking symptom is gone; stages a+b shipped)
**Progress 2026-05-30 (#223):** **stage (b) — vitest 2.1.9 → 3.2.4 (+ coverage-v8 lockstep) — SHIPPED.** vite held at 5.4.21 (vitest 3 carries vite as a direct dep `^5||^6||^7` → dedupes to vite 5, no dual-vite). vitest 3 narrowed the `test:`-block module augmentation on `defineConfig` from `'vite'`, so the import was swapped to **`'vitest/config'`** (already done — stage c no longer needs it). Suite green with ZERO test changes: 1020/1020, typecheck, both builds, 30 e2e. **Non-gating coverage drift:** coverage-v8 3's AST-aware remapping (`ast-v8-to-istanbul`) measures ~92% lines/stmts, ~88.7% fns vs the 95/94/88/95 floors; CI runs `vitest run` (no coverage) so it doesn't gate; thresholds left untouched → see **COVERAGE-1** (§5). Plan: `docs/plans/2026-05-29-ops-2b-vitest-3.md`.
**Progress 2026-05-29 (#221):** **stage (a) — vitest 1.6.1 → 2.1.9 — SHIPPED.** vite held at 5. Suite green on vitest 2's `forks` pool, zero config/test changes: 1020/1020. Plan: `docs/plans/2026-05-29-ops-2a-vitest-2.md`.
**Stage (c) reality (revised 2026-05-30):** vite 5→6 + esbuild 0.21→0.25, **keeping vitest at 3** (vitest 3 already accepts vite 6 — no vitest 4 needed; vitest 3→4 would be an optional later bump and 4 *requires* vite 6). The `'vitest/config'` import is already in place from stage (b), so the dual-vite typing risk is resolved. This is the slice that clears the OSV time-box allowlist.
**Update 2026-05-29 (#215):** the OSV-red symptom was resolved by **time-boxing the dev-only advisories** in `osv-scanner.toml` (#215) — the Security gate is now GREEN on main and OSV can join OPS-1's required set without the dep upgrade. So this dep-major upgrade is **no longer urgent**; it's now the *durable* fix that lets us **remove the time-box allowlist** before it expires. Staged approach below still applies. Priority dropped Medium→Low.
...
**Done when:** OSV Scanner job reports SUCCESS on main with zero advisories; OSV can be added to OPS-1's required-checks list; production bundle still builds for both Chrome and Firefox targets; all ~930 unit tests still green; e2e suite green.
