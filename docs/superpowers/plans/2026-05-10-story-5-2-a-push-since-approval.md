...
- **`requestReviewers` 422 handling** — POST is NOT a silent no-op on duplicate logins; it returns 422. The endpoint now distinguishes `alreadyRequested` (idempotent success) from other 422s (real error).
