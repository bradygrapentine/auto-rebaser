...
- **Store packaging + submission**: build store zips with `npm run build:store` (`STORE=1`), NEVER `build:all` — `STORE=1` strips the Chrome dev `key` that the repo `manifest.json` carries (CWS hard-rejects a mismatched key: "key field value doesn't match the current item"). Firefox MV3 requires `content_security_policy` as an **object** (`{ "extension_pages": "…" }`), not the MV2 string form, or AMO fails validation (`"/content_security_policy" must be object`). Before any submission, validate the BUILT manifest per target: Chrome zip has no `key` (`unzip -p …chrome….zip manifest.json | grep -c '"key"'` → 0), Firefox CSP is an object. Both are silent until the store rejects. Store copy source of truth: `marketing/store-listing-description.md`. Runbook: `docs/runbooks/2026-06-02-v2.1.0-release.md`. (OPS-6 will fold the key-strip into a `package:chrome` script.)
...
- **Shared-helper extraction must wire BOTH paths, not just the new consumer.** When you extract a helper specifically to enforce a "preview and execute feed `decide*` the same adapted objects" (or any "both paths use the same X") invariant — e.g. `buildEligiblePRs`/`buildMergedPRInputs` in `orchestrator.ts`, or the `decide*` predicates behind both `run*` and the preview branch — re-point the OLD path to call it in the SAME change. Wiring only the new consumer leaves a byte-equivalent duplicate on the execute path that can silently drift; the characterization wall passing only proves they match *now*, not that they're unified. If the old path can't be safely re-pointed in that PR, say so and file the rewire as an explicit follow-up — don't imply the paths are unified. **When you WANT a behavior divergence on only ONE path** (e.g. preview degrades per-PR on a flaky `getRepo`, execute keeps all-or-nothing abort), gate it with an explicit default-off flag on the shared helper (`degradePerPR = false`) and pin BOTH halves: the divergent-path test AND a "this path is unchanged" counter-test. The char wall proves the shared build is byte-identical; it does NOT prove the flag's branch behaves — and if the wall never exercises the divergent code path (e.g. it never rejects `getRepo` through the enable step), the counter-test is the ONLY guard against the divergence leaking into the preserved path. Past incident 2026-06-01 (PREVIEW-1 T2 → PREVIEW-7, #272): `buildEligiblePRs`/`buildMergedPRInputs` were extracted for preview but execute kept its inline build; PREVIEW-7 unified them and added the `degradePerPR` flag (preview-only) + an execute-unchanged counter-test, deferring execute's own degradation to PREVIEW-10.
