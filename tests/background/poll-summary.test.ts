...
describe('buildPollSummary', () => {
  it('produces correct counts from non-zero inputs', () => {
    ...
    expect(summary.notificationsDismissed).toBe(7);
    expect(summary.errors).toBe(1);
  });

  it('ranAt is within 100ms of call time', () => {
    const before = Date.now();
    const summary = buildPollSummary(0, { branchesDeleted: 0, autoMergeEnabled: 0, threadsResolved: 0, notificationsDismissed: 0 }, 0);
  ...
  it('all-zero inputs produce all-zero summary (except ranAt)', () => {
    ...
    expect(summary.notificationsDismissed).toBe(0);
    expect(summary.errors).toBe(0);
    expect(typeof summary.ranAt).toBe('number');
  });

  it('errors=0 still produces a valid summary', () => {
    const summary = buildPollSummary(1, { branchesDeleted: 1, autoMergeEnabled: 1, threadsResolved: 1, notificationsDismissed: 1 }, 0);
    expect(summary.errors).toBe(0);
