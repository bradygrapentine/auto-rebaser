...
describe('AccountSwitcher', () => {
  ...
  describe('cross-account action-dot', () => {
      ...
      render(
        <AccountSwitcher
          {...setup({
            accounts: [
              account(),
              account({ id: 'gh_work', login: 'work', actionableCount: 2 }),
            ],
          })}
        />,
      );
    ...
    it('does NOT render pill attention dot when only the active account has actionableCount > 0', () => {
      render(
        <AccountSwitcher
          {...setup({
            accounts: [
              account({ actionableCount: 3 }),
              account({ id: 'gh_work', login: 'work', actionableCount: 0 }),
            ],
          })}
        />,
      );
    ...
    it('renders dropdown row dot only for non-active accounts with actionableCount > 0', () => {
      render(
        <AccountSwitcher
          {...setup({
            accounts: [
              account({ actionableCount: 0 }),
              account({ id: 'gh_work', login: 'work', actionableCount: 1 }),
            ],
          })}
        />,
      );
      fireEvent.click(screen.getByRole('button', { name: /Account octocat/i }));
      expect(
        screen.queryByTestId('account-switcher-row-attention-gh_octocat'),
      ).not.toBeInTheDocument();
      expect(
        screen.getByTestId('account-switcher-row-attention-gh_work'),
      ).toBeInTheDocument();
    });

    it('does NOT render dropdown row dot for the active account even if it has actionableCount > 0', () => {
      render(
        <AccountSwitcher
          {...setup({
            accounts: [
              account({ actionableCount: 5 }),
              account({ id: 'gh_work', login: 'work', actionableCount: 0 }),
            ],
          })}
        />,
